...
func SubmitEncryPythonByCid(c *gin.Context) {
	...
	runbash := CreateBASHRunByCid(intcid, secretdata)
	...
	//复制attack.py到选手文件夹
	attackfilepath := "/home/ctf/codefile/attack" + cid + ".py"
	exec.Command("cp", "-r", attackfilepath, tokenpath).Run()
...
func CreateBASHRunByCid(cid int, secret string) string {
	//将secret以,分割
	...
	for i, _ := range secretlist {
		bash += fmt.Sprintf("python attack%d.py %s\n", cid, "png/"+strconv.Itoa(i+1)+"_encry.png")
